...
class tournamentShipControl : ShipForcesInducer, ServerShipUser, Ticked {
    ...
    private val anchorSpeed = tournamentConfig.SERVER.anchorSpeed
    ...
    override fun applyForces(physShip: PhysShip) {
        ...
        physShip.doFluidDrag = tournamentConfig.SERVER.doFluidDrag
        ...
        // Revisiting tournament control code.
        ...
        if (aligning && abs(angleUntilAligned) > ALIGN_THRESHOLD) {
            ...
            val idealOmega = Vector3d(invRotationAxisAngle.x, invRotationAxisAngle.y, invRotationAxisAngle.z)
                ...
                .mul(tournamentConfig.SERVER.stabilizationSpeed)
        ...
        controlData?.let { control ->
            ...
            var rotationVector = Vector3d(
                ...
                if (control.leftImpulse != 0.0f)
                    (control.leftImpulse.toDouble() * tournamentConfig.SERVER.turnSpeed)
                else
                    -omega.y() * tournamentConfig.SERVER.turnSpeed,
                0.0
            )
            ...
            val PitchVector = rotationVector.mul((control.forwardImpulse.toDouble()* tournamentConfig.SERVER.turnSpeed) * tournamentConfig.SERVER.turnSpeed * -1.5, Vector3d())

            rotationVector.mul(control.leftImpulse.toDouble() * tournamentConfig.SERVER.turnSpeed * -1.5)
            ...
            println(PitchVector)
            println()
            forcesApplier.applyInvariantTorque(PitchVector.rotateY(90.0))
            forcesApplier.applyInvariantTorque(rotationVector.mul(0.1))
            // endregion
            ...
            val baseForwardVel = Vector3d(forwardVector).mul(tournamentConfig.SERVER.baseSpeed)
            ...
            val idealForwardVel = Vector3d(forwardVector).mul(tournamentConfig.SERVER.maxCasualSpeed.toDouble())
            ...
            //forcesApplier.applyInvariantTorque(actualExtraForce.rotateY(90.0))
            // endregion
            ...
            if (control.upImpulse != 0.0f) {
                idealUpwardVel = Vector3d(0.0, 1.0, 0.0)
                    ...
                    .mul(tournamentConfig.SERVER.impulseElevationRate.toDouble())
            }
        }
    ...
    companion object {
        ...
        private val forcePerBalloon get() = tournamentConfig.SERVER.massPerBalloon * -GRAVITY
