...
fun stabilize(
    ...
    stabilizationTorque.mul(tournamentConfig.SERVER.stabilizationTorqueConstant)
    ...
    if (linear) {
        ...
        if (idealVelocity.lengthSquared() > (tournamentConfig.SERVER.linearStabilizeMaxAntiVelocity * tournamentConfig.SERVER.linearStabilizeMaxAntiVelocity))
            idealVelocity.normalize(tournamentConfig.SERVER.linearStabilizeMaxAntiVelocity.toDouble())

        idealVelocity.mul(ship.inertia.shipMass * (10 - tournamentConfig.SERVER.antiVelocityMassRelevance))
